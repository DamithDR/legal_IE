"""Abstract base class for LLM-based Event Detection evaluators."""
...
import config
from models.event_prompt_builder import build_event_system_prompt, build_event_user_prompt
from models.event_response_parser import parse_event_response
from evaluation.metrics import compute_ner_metrics


class LLMEventEvaluator(ABC):
    """Base class for LLM Event Detection evaluators."""
    ...
    def evaluate_sample(self, tokens: list[str], few_shot_examples: list[dict],
                        dataset_name: str = "events_matter") -> list[str]:
        """
        ...
        Returns:
            Predicted BIO tags.
        """
        user_prompt = build_event_user_prompt(tokens, few_shot_examples)
        system_prompt = build_event_system_prompt(dataset_name)
        response = self.call_api(system_prompt, user_prompt)
        pred_tags = parse_event_response(response, tokens)
        return pred_tags
    ...
    def evaluate_dataset(
        ...
        print(f"\nEvaluating Event Detection {provider_name} ({self.model_name}) on {len(samples)} samples...")

        for i, sample in enumerate(tqdm(samples, desc=f"Event {provider_name}")):
        ...
        print(f"\nEvent {provider_name} Results ({failures} failures out of {len(samples)}):")
